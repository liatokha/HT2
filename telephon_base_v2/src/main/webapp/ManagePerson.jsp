...
<%
    ...
    String user_message = "";
    String current_action = request.getParameter("action");
    ...
    <table align="center" border="1" width="70%">
        ...
        <tr>
            ...
            <td>
                ...
                <div style="overflow: auto;">
                    <div style="float: left; margin-right: 15px"><%=phone.getNumber()%>
                    </div>
                    <div style="float: left; margin-right: 15px;">
                        <a href="<%=request.getContextPath()%>/ManagePhone/?action=edit_phone&personId=<%=person.getId()%>&phoneId=<%=phone.getId()%>">Редактировать</a>
                    </div>
                    <div style="float: left; margin-right: 15px;">
                        <a href="<%=request.getContextPath()%>/ManagePhone/?action=delete_phone&personId=<%=person.getId()%>&phoneId=<%=phone.getId()%>">Удалить</a>
                    </div>
                </div>
                <%}%>
                <div>
                    <a href="<%=request.getContextPath()%>/ManagePhone/?action=add_phone&personId=<%=person.getId()%>">Добавить</a>
                </div>
            </td>
        </tr>
        <% } %>
        <tr>
            <td colspan="2" align="center">
                ...
                <div>
                    <a href="<%=request.getContextPath()%>/ManagePerson/">Вернуться к списку</a>
                </div>
            </td>
        </tr>
    </table>
